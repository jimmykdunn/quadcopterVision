...
def confusionMatrixNumToPct(confusionMatrix):
    ...
    fpRate = confusionMatrix[1]/(confusionMatrix[0]+confusionMatrix[1]) # noise called target/total targets
    fnRate = confusionMatrix[2]/(confusionMatrix[2]+confusionMatrix[3]) # target called noise/total noise
...
def runBasicPerformanceAnalysis(modelPath):
    ...
    confMatrix = calculateConfusionMatrix(heatmaps, y_all, 0.5)
