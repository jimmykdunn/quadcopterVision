...
def run_videoInjection(greenVideoFile, backgroundVideoFile, bgRotate_deg=0,
    ...
    try:
        ...
        while success_g and success_b:
            ...
            # Save off original frame for inspection
            cv2.imwrite(os.path.join("sequences",outVideoBase,'rawFrame_%04d.jpg' % iFrame), greenFrame)
            
            # Save off background frame for inspection
            cv2.imwrite(os.path.join("sequences",outVideoBase,'backFrame_%04d.jpg' % iFrame), backgroundFrame)
...
if __name__ == "__main__":
    ...
    run_videoInjection("defaultGreenscreenVideo.avi", os.path.join("backgroundVideos","roboticsLab1.avi"),bgRotate_deg=270,bgScale=0.33, shift=[300,50],label='junk')
